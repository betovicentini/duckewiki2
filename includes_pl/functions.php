...
function sec_session_start() {
	...
	if (ini_set('session.use_only_cookies', 1) === FALSE) {
		//CAMINHO PRECISA SER COLOCADO EM ARQUIVO DE CONFIGURACAO A SER CHAMADO POR ESTE ARQUIVO
		header("Location: $pathtoplantas/error.php?err=Could not initiate a safe session (ini_set)");
		exit();
	}
	...
	if (isset($_SESSION['user_id'])) {
		...
		if ($res) {
			if ($row = pg_fetch_array($res,null,PGSQL_ASSOC)) {
				...
				$_SESSION['cfg.show'] = $row['show'];				// mostra todos os registros, ou só aqueles em uso?
...
//DEFINE FUNCAO PARA EXTRAIR XML DO MYCOBANK E MOBOT
