...
$showQuery = false;
/*
 * Por que não usar pg_num_rows() ao invés de fazer duas queries? ??
 * 
 * Ordenar:
		...
		if (strpos($key,'txtFilter') === 0) {
			...
			if (in_array($table,['checklist','especimenes','plantas','locais','locais2'])) {
				switch ($key) {
					...
					case 'lat' :
						switch ($table) {
							case 'especimenes' : $key = 'e.lat';
								break;
							case 'plantas' : $key = 'pl.lat';
								break;
						}
						break;
					case 'lon' :
						switch ($table) {
							case 'especimenes' : $key = 'e.lon';
								break;
							case 'plantas' : $key = 'pl.lon';
								break;
						}
						break;
					case 'localidade' :
			...
			while (in_array(substr($value,0,1),['=','>','<','!','*'])) {
				$modif .= substr($value,0,1); // passa o modificador de $value para $modif
				$value = substr($value,1);
			}
			if ($modif == '') {
				$res = $res." unaccent(TEXT($key)) ilike unaccent('%$value%')"; // TEXT() para poder filtrar colunas numéricas
			} else {
				//echo "modif: $modif<BR>";
				if ($modif == '!') { // Comparação usando <>
					...
					if (in_array($value,['NULL','null'])) {
						...
						if ($modif == '*') {
							// pega todos os descendentes da query original
							$res = $res." $key = any(gettaxfilhos($value))";
						} else {
...
if (!empty($qry)) {
	...
	tt.nome pai,ttt.nome avô,r.nome rank,plantas,especímenes,t.nesp,t.npl from agg
	left outer join tax t on agg.tax = t.id
	left join tax tt on t.taxpai = tt.id
	left join tax ttt on tt.taxpai = ttt.id
	left join ranks r on r.id = t.rank
	left join pess p on p.id = t.aut";*/
		$q = "select t.id,t.nome táxon,
		case when (t.aut is null) then coalesce(t.nome,'')||' '||coalesce(t.auttxt,'') else t.nome||' '||p.abrev end as táxon_autor,
		tt.nome pai,ttt.nome avô,r.nome rank,t.nesp especímenes,t.npl plantas from tax t
		left join tax tt on t.taxpai = tt.id
		left join tax ttt on tt.taxpai = ttt.id
		left join ranks r on r.id = t.rank
		left join pess p on p.id = t.aut";
	...
	if ($table == 'especimenes') {
		if ($_SESSION['cfg.show'] == 'U') { // in Use
		} else { // All
		}
		$q = "select count(*) from (select e.id
		, e.ano, e.mes, e.dia
		, e.ano::text || to_char(e.mes, 'FM-00')
	...
	from esp e
	join prjesp pe on pe.esp = e.id and pe.prj in (
		select prj from prjpess pp
		join usr u on u.id = ".$_SESSION['user_id']."
		where pp.pess = u.pess and ver = 'S')
	left join pl on pl.id = e.pl
	left join det d on e.det = d.id
	left join pess p on p.id = d.detby
	left join var v on v.esp = e.id and v.key = 265
	...
	if ($table == 'plantas') {
		...
		if ($filtro == '') {
			// se não tiver filtro
			$q = "select count(*) from (select pl.pltag
	from pl
	join prjpl pp on pp.pl = pl.id and pp.prj in (
		select prj from prjpess pe
		join usr u on u.id = ".$_SESSION['user_id']."
		where pe.pess = u.pess and ver = 'S')
	left join det d on pl.det = d.id";
			$q = $q.$filtro.") as tbplantas";
		} else {
			// com filtro tem que permitir tax
			$q = "select count(*) from (select pl.id, (select count(id) from imgitem ii where ii.tabid = pl.id and ii.tab = 'pl') as imgs
	, pl.pltag tag
	, t.id tax
	, t.nome táxon
	, r.nome rank
	, gettax_fam(d.tax) família
	, gettax_gen(d.tax) gênero
	, gettax_sp(d.tax) espécie
	, pl.loc
	, l.nome localidade
	, d.ano anodet
	...
	left join prj on prj.id = pl.prj";
		...
		$row = pg_fetch_row($res);
		$rowcount = $row[0];
		$q = "select pl.id, (select count(id) from imgitem ii where ii.tabid = pl.id and ii.tab = 'pl') as imgs
	, pl.pltag tag
	, t.id tax
	, t.nome táxon
	, r.nome rank
	...
	if ($table == 'msg') {
		...
		msg.adddate,msg.thread,msg.lida,msg,url
		from msg,usr
...
if ($mostrar) { // daqui vai até o final
			...
			foreach ($row as $key => $value) {
				switch ($table) {
					...
					case 'locais' :
						...
						break;
					case 'locais2' :
						if (strpos($key,'tloc2') === 0) {
							$colunas[substr($key,5)] = $value;
						}
						break;
				}
			}
	...
	if ($onlycols) {
		...
		if ($res) {
			if ($row = pg_fetch_array($res,NULL,PGSQL_ASSOC)) {
				...
				//echo "Testando 1<BR>Rodrigo<BR>Testando 1<BR>Rodrigo<BR>Testando 1<BR>Rodrigo<BR>Testando 1<BR>Rodrigo<BR>";
				//print_r($colunas);
				//echo "Testando 2<BR>";
				//echo "$colunas<BR>";
				echo "[$cols]";
	...
	/////////////////////////////////////////////////////
		...
		//$rowcount = pg_num_rows($res); // não dá pra fazer isso por causa do LIMIT na query principal
		...
		if (!in_array($mark,['','showMark'])) { // se tiver que marcar/desmarcar...
			...
			if (!$export) { // se exportar o cabeçalho é outro
				...
				echo "<div id='divFundoHead'>\n";
				echo "<div id='divTHead'>\n";
				echo "<form id='frmTable' name='frmTable' method='get' action=''>\n";
				////////////////////////////////////////////////////////////////
				// DESENHA A TABELA - Cabeçalho - nomes das colunas+botões
				////////////////////////////////////////////////////////////////
				echo "<table id='tblHead' border='1' onMouseMove='row1MouseMove(event)' onMouseDown='row1MouseDown(event)' onMouseUp='row1MouseUp(event)' onContextMenu='showMenu(event,\"$table\");return false;'>
				<tr id='tr1Main' style='background-color:#".$_SESSION['cfg.cortbh']."'>";
				//<tr id='tr1Main' style='background-color:#".$_SESSION['cfg.cortbh']."' onMouseMove='row1MouseMove(event)' onMouseDown='row1MouseDown(event)' onMouseUp='row1MouseUp(event)' onContextMenu='showMenu(event,\"$table\");return false;'>";
				echo "<td>Edit</td>";
				echo "<td>Img</td>";
				...
				//echo "<tr id='tr2Main' style='background-color:#".$_SESSION['cfg.cortbh']."' onMouseMove='row1MouseMove(event)' onMouseDown='row1MouseDown(event)' onMouseUp='row1MouseUp(event)' onContextMenu='showMenu(event,\"$table\");return false;'>";
				...
				echo "</tr></table></form></div></div>"; // frmTable / divTHead / divFundoHead
			} else {
				$cols = [];
				for ($i=0; $i<$colcount; $i++) { // botões
			...
			////////////////////////////////////////////////////////////////
			echo "<div id='divFundoBody' onscroll='fbodyScroll(this)'>\n";
			echo "<div id='divTBody'>\n";
			echo "<table id='tblBody' border='1' style='table-layout:fixed'>";
			while ($row = pg_fetch_array($res,NULL,PGSQL_ASSOC)) {
				if (!$export) {
					if ($table == 'msg' && $row['lida'] == 'N') { // colore de vermelho se for msg não lida
						echo "<tr style='color:#F00'>";
					} else {
						echo "<tr>";
					}
					echo "<td style='text-align:left'><span style='white-space:nowrap'>";
					...
					switch ($table) {
						case 'checklist' :
							...
							echo "<img src='icon16/cor/loc.png' height=$l width=$l style='cursor:pointer' onclick='mapSee(null,null,null,$row[id])' title='Ver localidade dos registros'>";
						...
						case 'especimenes' :
							...
							if ($row['lat'] != null && $row['lon'] != null) {
								echo "<img src='icon16/cor/loc.png' height=$l width=$l style='cursor:pointer' onclick='mapSee($row[lat],$row[lon],null,null)' title='Ver localidade do registro'>";
							} else
							if ($row['loc'] != null) {
								echo "<img src='icon16/cor/loc.png' height=$l width=$l style='cursor:pointer' onclick='mapSee(null,null,$row[loc],null)' title='Ver localidade do registro'>";
							}
							break;
						case 'plantas' :
